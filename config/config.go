...
const DefaultLocation = "/etc/pteranodon/config.yml"
...
// exposed by the Buddy webserver.
...
// from Buddy to the Panel.
type RemoteQueryConfiguration struct {
	// The amount of time in seconds that Buddy should allow for a request to the Panel API
	...
	// Buddy instance. A single request is initially made to the Panel to get this number
...
type SystemConfiguration struct {
	// The root directory where all of the pteranodon data is stored at.
	RootDirectory string `default:"/var/lib/pteranodon" yaml:"root_directory"`

	// Directory where logs for server installations and other buddy events are logged.
	LogDirectory string `default:"/var/log/pteranodon" yaml:"log_directory"`

	// Directory where the server data is stored at.
	Data string `default:"/var/lib/pteranodon/volumes" yaml:"data"`

	// Directory where server archives for transferring will be stored.
	ArchiveDirectory string `default:"/var/lib/pteranodon/archives" yaml:"archive_directory"`

	// Directory where local backups will be stored on the machine.
	BackupDirectory string `default:"/var/lib/pteranodon/backups" yaml:"backup_directory"`

	// TmpDirectory specifies where temporary files for Pteranodon installation processes
	// should be created. This supports environments running docker-in-docker.
	TmpDirectory string `default:"/tmp/pteranodon" yaml:"tmp_directory"`

	// The user that should own all of the server files, and be used for containers.
	Username string `default:"pteranodon" yaml:"username"`

	// The timezone for this Buddy instance. This is detected by Buddy automatically if possible,
	// and falls back to UTC if not able to be detected. If you need to set this manually, that
	// can also be done.
	//
	// This timezone value is passed into all containers created by Buddy.
	...
	// impact system performance and cause massive I/O bottlenecks and high CPU usage for the Buddy
	// process.
	//
	...
	// If set to false Buddy will not attempt to write a log rotate configuration to the disk
...
type CrashDetection struct {
	...
	// Determines if Buddy should detect a server that stops with a normal exit code of
	// "0" as being crashed if the process stopped without any Buddy interaction. E.g.
...
type Backups struct {
	...
	// CompressionLevel determines how much backups created by buddy should be compressed.
	//
...
type Configuration struct {
	...
	// Determines if buddy should be running in debug mode. This value is ignored
	// if the debug flag is passed through the command line arguments.
	Debug bool

	AppName string `default:"Pteranodon" json:"app_name" yaml:"app_name"`
	...
	// is only required by users running Buddy without SSL certificates and using internal IP
...
// EnsurePteranodonUser ensures that the Pteranodon core user exists on the
...
func EnsurePteranodonUser() error {
	...
	log.WithField("username", _config.System.Username).Info("checking for pteranodon system user")
...
// EnableLogRotation writes a logrotate file for buddy to the system logrotate
...
func EnableLogRotation() error {
	if !_config.System.EnableLogRotate {
		log.Info("skipping log rotate configuration, disabled in buddy config file")
		return nil
	}
	...
	if _, err := os.Stat("/etc/logrotate.d/buddy"); err == nil || !os.IsNotExist(err) {
		return err
	}
	...
	// but there is not a file for buddy already. In that case, let us write a new file to
	// it so files can be rotated easily.
	f, err := os.Create("/etc/logrotate.d/buddy")
	if err != nil {
		return err
	}
	defer f.Close()

	t, err := template.New("logrotate").Parse(`{{.LogDirectory}}/buddy.log {
    size 10M
    compress
    delaycompress
    dateext
    ...
    notifempty
    postrotate
        /usr/bin/systemctl kill -s HUP buddy.service >/dev/null 2>&1 || true
    endscript
}`)
	if err != nil {
		return err
...
func ConfigureTimezone() error {
	...
	if _config.System.Timezone == "" {
		...
		if err != nil {
			...
			// fall through to UTC to get Buddy booted at least.
